...
var lint$1 = require('@codemirror/lint');
...
var cmTheme = require('@bpmn-io/cm-theme');
...
const feelBlock = new lr.ExternalTokenizer((input, stack) => {
...
const simpleTextBlock = new lr.ExternalTokenizer((input, stack) => {
...
const feel = new lr.ExternalTokenizer((input, stack) => {
...
const feelersHighlighting = highlight.styleTags({
  ConditionalSpanner: highlight.tags.special(highlight.tags.bracket),
  ConditionalSpannerClose: highlight.tags.special(highlight.tags.bracket),
  ConditionalSpannerCloseNl: highlight.tags.special(highlight.tags.bracket),
  LoopSpanner: highlight.tags.special(highlight.tags.bracket),
  LoopSpannerClose: highlight.tags.special(highlight.tags.bracket),
  LoopSpannerCloseNl: highlight.tags.special(highlight.tags.bracket),
  EmptyInsert: highlight.tags.special(highlight.tags.bracket),
  Insert: highlight.tags.special(highlight.tags.bracket),
});
...
const buildNodeEvaluator = (options) => {
  ...
  const evaluateNodeValue = (node, context = {}) => {
    ...
    case 'ConditionalSpanner': {
      ...
      try {
        shouldRender = feelin.evaluate(feel, context);
      }
      catch {
        return errorHandler(new Error(`FEEL expression ${feel} couldn't be evaluated`));
      }
    ...
    case 'LoopSpanner': {
      ...
      try {
        loopArray = feelin.evaluate(feel, context);
      }
      catch {
        return errorHandler(new Error(`FEEL expression ${feel} couldn't be evaluated`));
      }
...
const foldMetadata = {
  ConditionalSpanner: language.foldInside,
  LoopSpanner: language.foldInside
};
  ...
  const _mixedParser = parser.configure({
    ...
    wrap: common.parseMixed(node => {
      ...
      if (node.name == 'Feel' || node.name == 'FeelBlock') {
        return { parser: lezerFeel.parser };
      }
    ...
    props: [
      language.foldNodeProp.add(foldMetadata)
    ]
  });

  return language.LRLanguage.define({ parser: _mixedParser });
}

const createFeelersLanguageSupport = (hostLanguageParser) => new language.LanguageSupport(createMixedLanguage(hostLanguageParser), []);
...
function cmFeelersLinter() {
  const lintFeel = feelLint.cmFeelLinter();
  return editorView => {
...
var lint = lint$1.linter(cmFeelersLinter());
...
function FeelersEditor({
  ...
  const lintHandler = view.EditorView.updateListener.of((update) => {
    const diagnosticEffects = update.transactions
      .flatMap(t => t.effects)
      .filter(effect => effect.is(lint$1.setDiagnosticsEffect));
  ...
  const extensions = [
    ...
    lintHandler,
    tooltipLayout,
    darkMode ? cmTheme.darkTheme : cmTheme.lightTheme,
    ...(enableGutters ? [
    ...
    ...(singleLine ? [
      state.EditorState.transactionFilter.of(tr => tr.newDoc.lines > 1 ? [] : tr)
    ] : []),
    ...(lineWrap ? [
